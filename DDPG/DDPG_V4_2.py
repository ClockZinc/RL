...
尝试修改网络参数
from
input 64+dense 64+dense 1+sigmoid 1+(layer_Lambda)
to
input 32+dense 16+dense 1+tanh 1+(layer_Lambda)
'''
...
class DDPG(object):
    ...
    def __init__(self,s_dim,a_dim,a_bound):
        ...
        def get_actor(input_state_shape,output_shape,name=''):
            ...
            x=Dense(units=32,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer1')(inputs)
            x=Dense(units=16,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer2')(x)
            x=Dense(units=1,activation='tanh',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_sigmoid_layer')(x)
            x=layer_Lambda(lambda x:ub*x,name=name+'_Lambda_layer')(x)
        ...
        def get_critic(input_s_shape,input_a_shape,name=''):
            ...
            x=Dense(units=32,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer1')(x)
            x=Dense(units=16,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer2')(x)
