...
class DDPG(object):
    ...
    def __init__(self,s_dim,a_dim,a_bound):
        ...
        self.replay_buffer=np.zeros((REPLAYBUFFER_SIZE,s_dim*2+a_dim*1+2),dtype=np.float32)
    ...
    def store_transition(self,s,a,r,s_,d):
        ...
        transition=np.hstack((s,a,r,s_,d))# 横向堆叠这些变量
    ...
    def choose_transiton(self):
        ...
        br = bt[:, -self.s_dim - 2:-self.s_dim-1]         #从bt获得数据r
        bs_ = bt[:, -self.s_dim -1:-1]
        bd = bt[:,-1]
        bd = bd.reshape(-1,1) # 需要变成列向量，方便后续计算y
    ...
    # 通过tf.function 提升求导时间
    @tf.function #提升performance，减少训练时间
    def update_critic(self,bs,ba,br,bs_,bd):
    ...
    def learn(self):
        ...
        '''
        bs,ba,br,bs_,bd=self.choose_transiton()

        # 更新critic先
        self.update_critic(bs,ba,br,bs_,bd)
...
if args.train:
    ...
    for i in range(MAX_EPISODES):
        ...
        if reward_buffer:
            ...
            plt.ylim(-2000,0)# MAX_EP_STEPS=5000 -50000 # MAX_EP_STEPS=200 -2000
            plt.show()
            plt.pause(0.1)
    print('\n***end***')
    plt.ioff()    
    ...
    ddpg.save_ckpt()
    ddpg.save_pic(plt)
    ddpg.save_log(t=(time.time() - t0),r=reward_buffer)
