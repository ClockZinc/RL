...
'''
只是测试用
需要输入--path model file来指定测试的model
'''
...
parser.add_argument('--test', dest='test', action='store_false') # 未键入--test时，args.test=True
parser.add_argument('--seed',dest='seed',default=3407,type=int) # 未键入--seed 123 时，args.seed=3047
parser.add_argument('--path',dest='path',default='model',type=str) # 路径
...
class DDPG(object):
    ...
    def load_ckpt(self):
        ...
        :return: None
        """
        print('******\ncurrent path:{}'.format(args.path))
        self.actor.load_weights(args.path+'/ddpg_actor.hdf5')
        self.actor_target.load_weights(args.path+'/ddpg_actor_target.hdf5')
        self.critic.load_weights(args.path+'/ddpg_critic.hdf5')
        self.critic_target.load_weights(args.path+'/ddpg_critic_target.hdf5')
