...
class SAC_Actor_Model(tf.keras.Model):
    ...
    def __init__(self,s_dim,a_dim, W_init_coe=0.1, b_init_coe=0.001,log_std_min=-10, log_std_max=2, name=''):
        ...
        self.feature1           = Dense(64, activation=tf.nn.relu,kernel_initializer=W_init, bias_initializer=b_init)
        self.feature2           = Dense(64, activation=tf.nn.relu,kernel_initializer=W_init, bias_initializer=b_init)
...
def get_critic(s_dim,a_dim,name=''):
    ...
    x=Dense(units=32,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer2')(x)
...
def get_log_prob(x,mu,sigma):
    ...
    '''
    log_prob=- (x - mu)**2 / (2 * sigma**2 ) - tf.math.log(sigma) - log_sqrt_2PI
    return log_prob
...
class SAC(object):
    ...
    def update_actor(self,bs):
        with tf.GradientTape() as tape:
            ...
            Q_value_2       = self.q_value_2([bs,action])
            Q_min           = tf.math.minimum(Q_value_1,Q_value_2)
            Policy_loss     = tf.reduce_mean(log_prob-Q_min)
