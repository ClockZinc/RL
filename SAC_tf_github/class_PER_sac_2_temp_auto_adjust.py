...
# reward scaled
# references :
# https://blog.csdn.net/qq_36546042/article/details/119386805
...
# this version has totally 5 networks and can adjust alpha accordding to the entropy
...
from myfun import Memory_V1 as RepalyBuffer
from myfun import ema_V2,PI
...
class SAC_Actor_Model(tf.keras.Model):
    def __init__(self,s_dim,a_dim, W_init_coe=0.1, b_init_coe=0.001,log_std_min=-10, log_std_max=2,name=''):
        ...
        self.mu1                = Dense(256, activation=tf.nn.relu,kernel_initializer=W_init, bias_initializer=b_init)
        self.mu_out             = Dense(a_dim, activation='linear',kernel_initializer=W_init, bias_initializer=b_init)

        self.log_std1           = Dense(256, activation='relu',kernel_initializer=W_init, bias_initializer=b_init)
...
def get_critic(s_dim,a_dim,name=''):
    ...
    x=Dense(units=256,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer2')(x)
    x=Dense(units=256,activation='relu',kernel_initializer=W_init, bias_initializer=b_init,name=name+'_invisiable_layer3')(x)
...
class SAC(object):
    def __init__(self,s_dim,a_dim,a_bound,
        ...
        self.replay_buffer  = RepalyBuffer(REPLAYBUFFER_SIZE,s_dim=s_dim,a_dim=a_dim)
        if writer_mode:
        ...
        self.q_value_target_1      =get_critic(s_dim = s_dim,a_dim=a_dim,name='critic_target_1')
        self.q_value_target_2      =get_critic(s_dim,a_dim,name='critic_target_2')

        # 赋值,初始化actor的输入端
        copy_para(self.q_value_1,self.q_value_target_1)
        copy_para(self.q_value_2,self.q_value_target_2)
        # init actor
        ...
        # 初始化自动调整温度的算法
        self.tem_auto_adjust_flag = tem_auto_adjust_flag
        self.alpha_temp_opt = tf.optimizers.Adam(LR_C)
        self.alpha_entropy = tf.Variable(-a_dim,dtype=tf.float32)
        self.log_alpha  = tf.Variable(alpha)
    ...
    def choose_transition(self):
        bs,ba,br,bs_,bd,b_idx,ISWeights=self.replay_buffer.sample(self.BATCH_SIZE)
        return bs,ba,br,bs_,bd,b_idx,ISWeights
    '''
    cancel Value_network
    '''
    # @tf.function
    ...
    https://spinningup.openai.com/en/latest/algorithms/sac.html#pseudocode
    更新Qnetwork具体的流程为:
    对于下一时刻状态bs_,以及预估动作a_,我们可以得到
    两个Q——target的预测值
    于是target函数为:
    reward + \gamaa * (1 - d) * (min {Q_target} - \alpha * log_prob )
    where 
    ...
    def update_q_value(self,bs,ba,br,bs_,bd,ISWeights):
        ...
        Q_value_target_2 = self.q_value_target_2([bs_,action_])
        target_Q_value   = br + (1 - bd) * self.GAMMA * (tf.math.minimum(Q_value_target_1,Q_value_target_2)-self.alpha * log_prob_)
        with tf.GradientTape(persistent= True) as tape:         # 通过这种形式获得梯度信息
            Q_value_1       = self.q_value_1([bs,ba])
            Q_value_2       = self.q_value_2([bs,ba])
            td_error_1 = tf.reduce_mean( ISWeights * tf.math.squared_difference(Q_value_1, target_Q_value) )
            td_error_2 = tf.reduce_mean( ISWeights * tf.math.squared_difference(Q_value_2, target_Q_value) )
        Q_grads_1 = tape.gradient(td_error_1, self.q_value_1.trainable_weights) # 得到偏LOSS偏omega
        Q_grads_1 = [tf.clip_by_norm(g, 0.5) for g in Q_grads_1]
        Q_grads_2 = tape.gradient(td_error_2, self.q_value_2.trainable_weights) # 得到偏LOSS偏omega
        Q_grads_2 = [tf.clip_by_norm(g, 0.5) for g in Q_grads_2]
        self.q_value_1_opt.apply_gradients(zip(Q_grads_1, self.q_value_1.trainable_weights))# 进行一次梯度下降（目的是降低LOSS）
        self.q_value_2_opt.apply_gradients(zip(Q_grads_2, self.q_value_2.trainable_weights))# 进行一次梯度下降（目的是降低LOSS）
        del tape
        abs_td_error = tf.math.reduce_sum(tf.abs((Q_value_1+Q_value_2)/2-target_Q_value),axis=1)
        return abs_td_error
    ...
    def learn(self):
        # get transit
        bs,ba,br,bs_,bd,b_idx,ISWeights=self.choose_transition()

        abs_td_error = self.update_q_value(bs,ba,br,bs_,bd,ISWeights)

        if (self.pointer_update_times % self.POLICY_DELAY ==0):
            self.update_actor(bs,ISWeights)
            self.alpha = tf.Variable(tf.math.exp(self.log_alpha))
        
        self.replay_buffer.batch_update(b_idx,abs_td_error)

        # ema
        self.ema.update_target_network(network=self.q_value_1,target_network=self.q_value_target_1)
        self.ema.update_target_network(network=self.q_value_2,target_network=self.q_value_target_2)
    ...
    def save_ckpt(self):
        ...
        self.q_value_target_1.save_weights(self.model_name+'/'+self.algorithm_name+'_value.hdf5')
        self.q_value_target_2.save_weights(self.model_name+'/'+self.algorithm_name+'_value_target.hdf5')
    ...
    def load_ckpt(self,mode=False):
        ...
        if mode:
            self.q_value_target_1.load_weights(self.model_name+'/'+self.algorithm_name+'_value.hdf5')
            self.q_value_target_2.load_weights(self.model_name+'/'+self.algorithm_name+'_value_target.hdf5')
