...
from myfun import PrintProgressBar_init
from class_sac import SAC
...
BATCH_SIZE=1024           # learn per batch size
VAR=1                   # variance of the action for exploration
TAU=0.01                # soft update parameter 不能太小
    ...
    for i in range(MAX_EPISODES):
        ...
        print(
            '\rEpisode: {}/{}  | Episode Reward: {:.4f}  | Running Time: {:.4f}|est: {}h {:2.1f}min'.format(
                i, MAX_EPISODES, ep_reward,cost_time,int(hour),minute
            ), end=''
        )
        ...
        if i and not i % TEST_PER_EPISODES:
            ...
            for j in range(MAX_EP_STEPS):
                ...
                a = sac.choose_action(s)  # 注意，在测试的时候，我们就不需要用正态分布了，直接一个a就可以了。
                s_, r, done, info = env.step(a)
